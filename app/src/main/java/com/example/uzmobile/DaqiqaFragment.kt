...
import com.example.uzmobile.adapters.DaqiqaPagerAdapter
import com.example.uzmobile.adapters.XizmatlarPagerAdapter
...
class DaqiqaFragment : Fragment() {
    ...
    private lateinit var daqiqaPagerAdapter: DaqiqaPagerAdapter
    override fun onCreateView(
        ...
        loadCategories()
        daqiqaPagerAdapter = DaqiqaPagerAdapter(childFragmentManager, categoryList)
        root.daq_view_pager.adapter = daqiqaPagerAdapter
        root.daq_tab_layout.setupWithViewPager(root.daq_view_pager)
        setUpTabItems()
        root.daq_tab_layout.addOnTabSelectedListener(object : TabLayout.OnTabSelectedListener {
    ...
    private fun setUpTabItems() {
        val tabCount = root.daq_tab_layout.tabCount
        for (i in 0 until tabCount) {
            val tabAt = root.daq_tab_layout.getTabAt(i)
    ...
    private fun loadCategories() {
        ...
        categoryList.add("Constructor abonentlari uchun")
        categoryList.add("Daqiqa to'plamlar")
        categoryList.add("Foydali almashinuv")
    }
